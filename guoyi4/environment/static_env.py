...
from environment.lookup_tables import Fen_2_Idx_UCCI, flip_move
import numpy as np

# 初始状态，小写字母为黑方； 大写字母为红方
# r车，n马，b象，a士，k帅，c炮，p兵, ucci协议
...
def done(state, turns=-1, need_check=False):
    ...
    :param state:
    :param turns:
    :param need_check: 判断红方是否处于被将军状态
    :return:
    """
    # 没有将或者帅了
    if 'K' not in state:
...
def will_check_or_catch(ori_state, action):
    """
    自己写的
    判断ori_state 执行action的动作是否在捉对方子或者将对方军
    :param ori_state:
    :param action:
    :return:
    """
    board = state_to_board(ori_state)
    board = step_board(board, action)  # 执行动作但不翻转
    black_k = [0, 0]
    for i in range(BOARD_HEIGHT):
        for j in range(BOARD_WIDTH):
            if board[i][j] == 'K':  # 黑将所在位置
                black_k[0] = i
                black_k[1] = j
    red_moves = get_legal_moves(board)
    for mov in red_moves:
        dest = [int(mov[3]), int(mov[2])]
        if dest == black_k:
            return True
...
def will_check_or_catch2(ori_state, action):
    """
    判断ori_state 执行action的动作是否在捉对方子或者将对方军
    :param ori_state:
    :param action:
    :return:
    """
    state = step(ori_state, action)  # 执行动作但不翻转
    board = state_to_board(state)
    red_k = [0, 0]
    for i in range(BOARD_HEIGHT):
        for j in range(BOARD_WIDTH):
...
def get_catch_list(state, board=None, moves=None):
    catch_list = set()
    if not moves:
...
def step_board(board, action):
    """
    基于当前局面往前走一步，使得所有的考察都是基于红方的
    :param state:
    :param action:
    :return:
    """
    if board[int(action[1])][int(action[0])] == '.':
        raise ValueError("No chessman")
    board[int(action[3])][int(action[2])] = board[int(action[1])][int(action[0])]
    board[int(action[1])][int(action[0])] = '.'
    return board


def step(state, action, flip=True):
    """
...
def get_legal_moves(state, board=None):
    ...
    for y in range(BOARD_HEIGHT):
        for x in range(BOARD_WIDTH):
            ch = board[y][x]
            if not ch.islower():  # '.'和大写都是False，只基于小写字母来考虑
                continue
            if ch in mov_dir:
                for d in mov_dir[ch]:
                    x_ = x + d[0]
                ...
                l, r = x_board_from(board, x, y)  # 左右无字区间
                d, u = y_board_from(board, x, y)  # 上下无字的区间
...
def state_to_planes(state):
    """
    根据字符串代表的状态制造特征平面，shape是(14, 10, 9)， 前七个为大写字母，黑方，后七个小写字母。红方
    :param state:
    :return:
    """
    ...
    for i in range(len(rows)):
        ...
        for letter in row:
            ...
            if letter.isalpha():
                ...
                planes[Fen_2_Idx_UCCI[letter] + int(letter.islower()) * 7][i][j] = 1
                j += 1
            else:
                j += int(letter)
    return planes
