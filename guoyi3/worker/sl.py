...
class SupervisedWorker(object):
    ...
    def __init__(self, cfg):
        ...
        # self.dataset = deque(), deque(), deque(), deque()  # 局面，行动，价值，权重
        self.dataset = None
        self.buffer = []  # 数据缓存
        self.gameinfo = None
        self.moves = None
    ...
    def load_game(self, red, black, winner, idx):
        ...
        if winner == 'red':
            ...
            red_win = 0
        w = self.construct_weight(black_max_turn+red_max_turn)
        for i, move in enumerate(red_moves):
            move += [red_win, w[2*i]]
        for i, move in enumerate(black_moves):
            move += [-red_win, w[2*i+1]]
        data = []
    ...
    def construct_weight(self, length, gamma=0.95):
        """
    ...
    def convert_to_trainging_data(self):
        ...
        value_list = []
        w_list = []
        for state_fen, policy, value, w in data:
        ...
        return np.asarray(state_list, dtype=np.float32), np.asarray(policy_list, dtype=np.float32), np.asarray(
            value_list, dtype=np.float32), np.asarray(w_list, np.float32)
    ...
    def generate_game_data(self, games):
        ...
        for idx, game in games.iterrows():  # 读取一局游戏
            ...
            if len(red) > 0 and (len(red) == len(black) or len(red) == len(black)+1):
                try:
                    self.load_game(red, black, winner, idx)
                except:
                    from IPython import embed; embed()
            else:
                print("invalid game: ", gid)
    ...
    def fill_queue(self, games):
        ...
        _tuple = self.generate_game_data(games)  # 返回数据元组
    ...
    #     state_ary, policy_ary, value_ary, w_ary = self.dataset
    ...
    #     w_ary = np.asarray(w_ary, dtype=np.float32)
    #     return state_ary1, policy_ary1, value_ary1, w_ary

    def train_epoch(self, batch_size):
        # state_ary, policy_ary, value_ary, w_ary = self.collect_all_loaded_data()
        state_ary, policy_ary, value_ary, w_ary = self.dataset
        ...
        while idx + batch_size <= data_length:
            step, xentropy_loss, value_loss, total_loss = self.model.train_one_step(
                state_ary[idx: idx + batch_size], policy_ary[idx: idx + batch_size],
                value_ary[idx: idx + batch_size], w_ary[idx: idx + batch_size])
        ...
        if idx < data_length:
            step, xentropy_loss, value_loss, total_loss = self.model.train_one_step(
                state_ary[idx: data_length], policy_ary[idx: data_length],
                value_ary[idx: data_length], w_ary[idx: idx + batch_size])
            print("step %d, policy_loss %0.3f, value_loss %0.3f, total_loss %0.3f" % (
                step, xentropy_loss, value_loss, total_loss))
