...
def num_between(max_num, min_num, current):
    return min_num <= current <= max_num
...
class Rook(Chessman):
    ...
    def calc_moving_list(self):
        ...
        super(Rook, self).calc_moving_path(left, (left.position.x if left is not None else None),
                                           current_v_c, current_h_c, 1, 0, True)
        super(Rook, self).calc_moving_path(right, (right.position.x if right is not None else None),
                                           current_v_c, current_h_c, -1, 8, True)
        super(Rook, self).calc_moving_path(top, (top.position.y if top is not None else None),
                                           current_h_c, current_v_c, -1, 9, False)
        super(Rook, self).calc_moving_path(bottom, (bottom.position.y if bottom is not None else None),
...
class Cannon(Chessman):
    ...
    def calc_moving_list(self):
        ...
        tar_bottom = super(Cannon, self).chessboard.get_bottom_second_chessman(
            current_v_c, current_h_c)
        super(Cannon, self).calc_moving_path(left, (left.position.x if left is not None else None),
                                             current_v_c, current_h_c, 1, 0, True, True)
        super(Cannon, self).calc_moving_path(right, (right.position.x if right is not None else None),
                                             current_v_c, current_h_c, -1, 8, True, True)
        super(Cannon, self).calc_moving_path(top, (top.position.y if top is not None else None),
                                             current_h_c, current_v_c, -1, 9, False, True)
        super(Cannon, self).calc_moving_path(bottom, (bottom.position.y if bottom is not None else None),
