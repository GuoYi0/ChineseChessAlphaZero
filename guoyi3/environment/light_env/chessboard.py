...
class L_Chessboard(object):
    def __init__(self, init=None):
        """
        一个轻量级的游戏局面, 有子的地方用字母，没子的地方用'.'
        r车; n马； b象，a士，k帅，p兵，c炮
        :param init:
        """
        self.height = 10
        self.width = 9
    ...
    def parse_WXF_move(self, wxf):
        """
        :param wxf: 形如r6+4的一步棋。红棋大写，黑棋小写
        :return:
        """
        p = self.swapcase(wxf[0])  # 大小写切换
        col = wxf[1]
        mov = wxf[2]
        dest_col = wxf[3]
        src_row, src_col = self.find_row(p, col)  # 找到棋子所在行列
        if mov == '.' or mov == '=':
            # 水平走
            dest_row = src_row
            if p.islower():
    ...
    def find_row(self, piece, col):
        """
        形如r6+4的一步棋, piece是第一个字符，col是第二个字符
        即第6列的车。如果第六列只有一个车，则用数字6表示，否则用+-表示，-表示离己方近的那一行
        返回棋子所在的行列
        :param piece: r
        :param col: 6
        :return:
        """
        if piece == 'h' or piece == 'H':  # h -> n，马
            piece = 'n' if piece == 'h' else 'N'
        if piece == 'e' or piece == 'E':  # e -> b，象 同一棋子的两种表示
            piece = 'b' if piece == 'e' else 'B'
        column = 0
        row = -1
        if col.isdigit():  # 数字
            if piece.isupper():  # 红棋
                column = self.width - int(col)  # 红棋的编号是从左往右，故需要反过来
            else:
                column = int(col) - 1
            for i in range(self.height):
