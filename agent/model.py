...
class CChessModel(object):
    ...
    def train_one_step(self, data, policy_labels, value_labels):
        xentropy_loss, value_loss, total_loss, _, sum_res = self.sess.run(
            ...
            feed_dict={self.inputs: data, self.policy_labels: policy_labels,
                       self.value_labels: value_labels, self.training: True})
    ...
    def build_network(self):
        ...
        with tf.variable_scope("value"):
            ...
            self.value = tf.layers.dense(value, 1, activation=tf.nn.tanh, name="fc2")

    def build_loss_and_optimizer(self):
        self.xentropy_loss = tf.negative(
            tf.reduce_mean(tf.reduce_sum(tf.multiply(self.policy_labels, tf.nn.log_softmax(self.policy)), axis=1)))
        self.value_loss = tf.reduce_mean(tf.squared_difference(self.value, self.value_labels))
