...
class SupervisedWorker(object):
    ...
    def __init__(self, cfg):
        ...
        # self.loaded_data = deque(maxlen=self.config.Train.dataset_size)
    ...
    def load_game(self, red, black, winner, idx):
        ...
        :return:
        """
        env = CChessEnv(self.config).reset()  # 初始化一个游戏局面
        red_moves = []
        black_moves = []
        turns = 1
        ...
        while turns < black_max_turn or turns < red_max_turn:
            if turns < red_max_turn:
                ...
                action = env.board.parse_WXF_move(wxf_move)  # 解析走法 形如 r6+4 => 3478表示 (3,4)位置去棋子走到(7,8)
                try:
    ...
    def convert_to_trainging_data(self):
        ...
        for state_fen, policy, value in data:
            ...
            state_planes = env.fen_to_planes(state_fen)  # 返回np.zeros(shape=(14, 10, 9))的输入特征
            sl_value = value
